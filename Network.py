...
number_of_features_hidden_a = 25  #
...
W6 = tf.Variable(tf.truncated_normal([number_of_features_hidden_c, number_of_efectors], stddev=0.1), name="W6")
#
...
unnormalized_output = tf.add(tf.matmul(hidden_layer_b, W6), b6)
...
#add random noise
# def ornstein_uhlenbeck_function(x, mu, theta, sigma):
#     return tf.add(tf.multiply(theta, tf.add(mu, -x)), tf.multiply(sigma, tf.random_normal(tf.shape(x))))
#
# steering_normalized = tf.add(steering_normalized, ornstein_uhlenbeck_function(steering_normalized, 0.0, 0.60, 0.30))
# acceleration_normalized = tf.add(acceleration_normalized, ornstein_uhlenbeck_function(acceleration_normalized, 0.5, 1.00, 0.10))
# brake_normalized = tf.add(brake_normalized, ornstein_uhlenbeck_function(brake_normalized, -0.1, 1.00, 0.05))
